...
"""
Parse SMPS files. Particular emphasis has been put in maintaining sparse representation of the data, to allow
...
de = sm.generate_deterministic_equivalent()
de.optimize()

This expects the files './data/invest.cor', './data/invest.tim' and './data/invest.sto' to exist and follow the
SMPS format.
...
- A:    A dictionary of sparse matrices; each entry of A is a sub-block of the nominal model matrix A.
...
- b, c, lb, ub, vtype:  Also dictionaries, one entry in the dictionary per stage, and each entry a vector with the
                        nominal problem data.
- StochasticModel.scenario['SCENARIO_LABEL'].A: modified matrix A according to scenario 'SCENARIO_LABEL'. Same way
                                                b, c, lb, ub and vtype can be accessed. These are generated lazily.
- StochasticModel.generate_deterministic_equivalent():  returns a gurobi model for the deterministic equivalent of the
                                                        problem. Only available for models of type scenarios discrete.
...
class StochasticModel(object):
    ...
    def __init__(self, model_filename):
        ...
        self.stage_vars = {}
        self.stage_constrs = {}
        self.mode_of_modification = 'REPLACE'  # default, possible values supported: ['REPLACE', 'ADD']
    ...
    def plot_scenario_tree(self):
        ...
        node_position = nx.nx_pydot.graphviz_layout(G, prog='dot')
